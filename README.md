...
- **Asynchrony and Stability**: The process is delegated to the background (Celery worker), which does not block other parts of the application. The rate limit of 5 requests/s per e-shop API call is enforced via a synchronous delay between requests, which is sufficient for serial processing within a single worker. For distributed multi-worker setups, this could be upgraded to a **Redis-backed sliding window rate limiter** (atomic `INCR` + `EXPIRE` pipeline) or a **Token Bucket** pattern to enforce the global limit across all concurrent workers. Another improvement would be to dispatch each product as a separate Celery task (`sync_single_product`) so that 429 retries only re-send the affected product, not the entire batch.
- **Auto-Retries (HTTP 429)**: When the API returns "Too Many Requests", the system handles it with two strategies: (1) **Celery retry** with exponential backoff (2s, 4s, 8s...) when running as a Celery task, and (2) **synchronous fallback** with bounded retries and exponential backoff for direct calls. `CeleryRetry` exceptions are correctly propagated through the outer exception handler to ensure Celery can reschedule the task.
